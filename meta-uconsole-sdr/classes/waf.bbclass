...
# Full copy of oe-core's waf.bbclass, overriding poky's kirkstone-pinned
# copy (this layer's BBFILE_PRIORITY of 10 beats core meta's default of
# 5) to backport a fix from upstream master's meta/classes-recipe/waf.bbclass.
#
# kirkstone's waf_preconfigure() does
# `version = result.decode('utf-8').split()[1]`, blindly taking the
# second whitespace token of the *entire* combined stdout+stderr of
# `waf --version`. On this build host's Python (3.12), some waf-bundled
# scripts emit a SyntaxWarning line before the actual "waf X.Y.Z ..."
# version line, so that second token ends up being "SyntaxWarning:"
# instead of a version number, and bb.utils.vercmp_string_op() crashes
# trying to int-parse it as an epoch (do_configure failure on
# jack_1.19.20.bb and any other recipe that `inherit`s waf). Fixed
# upstream by scanning for the line that actually starts with "waf "
# and validating it's a real semver before using it. Confirmed
# bb.utils.is_semver() (used below) already exists in kirkstone's
# bitbake, so this backports cleanly with no other changes needed.
