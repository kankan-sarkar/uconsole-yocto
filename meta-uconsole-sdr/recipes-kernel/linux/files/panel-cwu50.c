...
static int cwu50_probe(struct mipi_dsi_device *dsi)
{
	...
	/*
	 * Ported from ClockworkPi's kernel fork (6.12-era struct
	 * drm_panel), which names this field prepare_prev_first. Our
	 * target (RPi Foundation's rpi-5.15.y, matching this project's
	 * linux-raspberrypi_5.15.bb) has the same boolean under the name
	 * prepare_upstream_first -- confirmed by reading the real
	 * include/drm/drm_panel.h on that branch before making this
	 * change. Same meaning: the DSI host must be prepared before this
	 * panel's own .prepare() runs.
	 */
